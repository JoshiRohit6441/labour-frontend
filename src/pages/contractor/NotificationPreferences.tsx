...
const ContractorNotificationPreferences = () => {
  ...
  useEffect(() => {
    contractorNotificationsApi.getPreferences().then((r) => {
      setPrefs(r.data || {});
    }).catch(() => {});
  }, []);
  ...
  const save = async () => {
    try {
      await contractorNotificationsApi.updatePreferences(prefs);
...
export default ContractorNotificationPreferences;
